...
void main() {
  ...
  float tile = 2.0;
  float speed = 0.01;
  // vec4 noise = texture2D(uNoiseTexture, abs(fract(sin(aTexCoord * tile + uFrameCount * speed))));
  vec4 noise = texture2D(uNoiseTexture, abs(fract(sin(uFrameCount * speed) + aTexCoord * tile)));
